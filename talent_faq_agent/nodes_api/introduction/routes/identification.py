...
from talent_faq_agent.nodes_api.utils import load_text_file, load_json_file, env_config
...
async def identification(
    ...
    client = AsyncOpenAI(base_url=env_config["llm_generation"]["url"],
                         api_key=env_config["llm_generation"]["API-key"])
    ...
    response = await client.chat.completions.create(
        model=env_config["llm_generation"]["model"],
        messages=task_messages,
        temperature=request_body.temperature,
        top_p=request_body.top_p
    )
